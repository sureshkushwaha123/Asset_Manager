...
export function ProfileDrawer({ open, onClose }: ProfileDrawerProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={v => !v && onClose()}>
      <SheetContent
        side="right"
        className="w-full sm:max-w-[480px] bg-background border-l border-white/10 flex flex-col p-0 overflow-hidden"
      >
        {/* Header */}
        <div className="bg-gradient-to-br from-primary/20 to-emerald-500/10 border-b border-white/10 p-6">
          <SheetHeader className="mb-4">
            <SheetTitle className="text-white text-lg">Account & Settings</SheetTitle>
          </SheetHeader>
          {isLoading ? (
            ...
            <div className="flex items-center gap-4">
              <div className="relative w-16 h-16 shrink-0">
                {avatarUrl ? (
                  <img src={avatarUrl} alt="Avatar" className="w-16 h-16 rounded-full object-cover border-2 border-primary/40" />
                ) : (
              ...
              <div>
                <p className="font-semibold text-white text-lg">{profile?.fullName || profile?.username}</p>
        ...
        <div className="flex-1 overflow-y-auto">
          <Tabs defaultValue="profile" className="h-full">
            <TabsList className="w-full rounded-none border-b border-white/10 bg-card/30 h-auto p-1 grid grid-cols-6 gap-0.5">
              {[
                ...
                <TabsTrigger
                  ...
                  className="flex flex-col items-center gap-0.5 py-2 px-1 text-[10px] data-[state=active]:bg-primary/20 data-[state=active]:text-primary rounded-lg"
                >
            ...
            {/* ── 1. PROFILE ── */}
            <TabsContent value="profile" className="p-5 space-y-5 mt-0">
            ...
            <TabsContent value="security" className="p-5 space-y-5 mt-0">
              <div className="rounded-xl bg-white/5 border border-white/10 p-4 space-y-1">
                <p className="text-xs text-muted-foreground flex items-center gap-1.5"><Activity className="w-3.5 h-3.5" /> Last Login</p>
              ...
              <form onSubmit={passwordForm.handleSubmit(d => changePassword.mutate(d))} className="space-y-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    />
                    <button type="button" onClick={() => setShowCurrent(s => !s)} className="absolute right-3 top-3 text-muted-foreground">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      className="bg-black/40 border-white/10 text-white h-11 rounded-xl pr-10"
                    />
                    <button type="button" onClick={() => setShowNew(s => !s)} className="absolute right-3 top-3 text-muted-foreground">
                  ...
                  <div className="rounded-xl bg-white/5 border border-white/10 divide-y divide-white/5">
                    {activity.slice(0, 5).map(a => (
                      <div key={a.id} className="flex items-center justify-between p-3">
                        ...
                        <p className="text-xs text-muted-foreground">{format(new Date(a.createdAt), "dd MMM, HH:mm")}</p>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <TabsContent value="prefs" className="p-5 space-y-5 mt-0">
            ...
            <TabsContent value="snapshot" className="p-5 space-y-4 mt-0">
              <p className="text-sm font-semibold text-white mb-1">Your Financial Overview</p>
              {!summary ? (
                  ...
                  {[
                    {
                      ...
                      value: `${profile?.defaultCurrency || "INR"} ${summary.currentBalance.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`,
                      icon: Wallet,
                      color: "text-primary",
                      bg: "bg-primary/10",
                    },
                    {
                      ...
                      value: `${profile?.defaultCurrency || "INR"} ${summary.totalMonthlySpending.toLocaleString("en-IN", { minimumFractionDigits: 2 })}`,
                      icon: TrendingUp,
                      color: "text-red-400",
                      bg: "bg-red-400/10",
                    },
                    ...
                    <div key={card.label} className={`rounded-xl ${card.bg} border border-white/10 p-4`}>
            ...
            {/* ── 5. NOTIFICATIONS ── */}
            <TabsContent value="notifs" className="p-5 space-y-5 mt-0">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    },
                    {
                      ...
                      label: "AI Insights",
                      desc: "Receive personalized AI financial recommendations",
            ...
            {/* ── 6. PRIVACY ── */}
            <TabsContent value="privacy" className="p-5 space-y-4 mt-0">
              ...
              <div className="rounded-xl bg-red-500/5 border border-red-500/20 p-4 space-y-3">
                ...
                <p className="text-xs text-muted-foreground">
                  Your account will be deactivated and you will be logged out. This action cannot be undone.
                </p>
                <AlertDialog>
                  ...
                  <AlertDialogContent className="bg-background border border-white/10">
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription className="text-muted-foreground">
                        Your account will be permanently deactivated. All data will be retained but you won't be able to log in. This cannot be undone.
